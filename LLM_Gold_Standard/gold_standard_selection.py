...
def concat_artist_track(artist, track):
    """Combine artist and track into a single string"""
    return f"{str(artist)} {str(track)}".lower().strip()
...
try:
    ...
    df_apple_xml['apple_combined'] = df_apple_xml.apply(
        lambda x: concat_artist_track(x['apple_Artist'], x['apple_Track']), axis=1
    )
    df_opendb_xml['opendb_combined'] = df_opendb_xml.apply(
        lambda x: concat_artist_track(x['opendb_Artist'], x['opendb_Track']), axis=1
    )
    df_million_xml['million_combined'] = df_million_xml.apply(
        lambda x: concat_artist_track(x['million_Artist'], x['million_Track']), axis=1
    )
