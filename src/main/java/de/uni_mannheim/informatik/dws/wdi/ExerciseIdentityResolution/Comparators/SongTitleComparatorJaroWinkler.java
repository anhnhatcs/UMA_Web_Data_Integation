 ...
 * {@link Comparator} for {@link Song}s based on {@link Song#getTrack()} values.
 * Created by Shamalan Rajesvaran
 * @version 1.1
 */
public class SongTitleComparatorJaroWinkler implements Comparator<Song, Attribute> {
    ...
    public double compare(
        ...
        String track1 = record1.getTrack();
        String track2 = record2.getTrack();

        // Calculate similarity using Jaro-Winkler similarity
        double similarityScore = similarity.calculate(track1, track2);

        if (this.comparisonLog != null) {
            this.comparisonLog.setComparatorName(getClass().getName());
            this.comparisonLog.setRecord1Value(track1);
            this.comparisonLog.setRecord2Value(track2);
